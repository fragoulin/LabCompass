...
QList<RoomId> NavigationData::plannedRouteInsideSection(int section, const RoomId& begin, const RoomId& end, const QList<RoomId>& targetRooms, ContentState* tempGolden)
{
    ...
    if (begin == end && std::count_if(sectionTargetRooms.constBegin(), sectionTargetRooms.constEnd(), [begin](const RoomId& target) { return target == begin; }) == sectionTargetRooms.size())
        return { begin };
    ...
    while (!queue.empty()) {
        ...
        for (auto i = connections.constBegin(); i != connections.constEnd(); i++)
            if (!i.value().isEmpty() && lab->sections[section].roomIds.contains(i.key())) {
                ...
                if (state.contentState.lockedDoors.contains(std::pair<RoomId, RoomId>(state.currentRoom, roomId)) || state.contentState.lockedDoors.contains(std::pair<RoomId, RoomId>(roomId, state.currentRoom))) {
